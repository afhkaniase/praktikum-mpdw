...
Berdasarkan uji ADF tersebut, didapat *p-value* sebesar 0.2873 yang lebih besar dari taraf nyata 5% sehingga tidak tolak $H_0$ dan menandakan bahwa data partisi bagian pertama tidak stasioner dalam rataan. Hal ini sesuai dengan hasil eksplorasi menggunakan plot time series dan plot ACF.

#### Plot Boxcox

```{r}
index <- seq(1:104)
bc = boxcox(dt_stas1~index, lambda = seq(-1,6,by=1))
#Nilai Rounded Lambda
lambda <- bc$x[which.max(bc$y)]
lambda
#SK
...
Gambar di atas menunjukkan nilai *rounded value* ($\lambda$) optimum sebesar **1.31** dan pada selang kepercayaan 95% nilai memiliki batas bawah **0.59** dan batas atas **2.04**. Selang tersebut memuat nilai satu sehingga dapat dikatakan bahwa data partisi bagian pertama stasioner dalam ragam.

#### Bagian 2

```{r}
dt_stas2 <- datalat5.ts[105:212] |> ts()
mean(dt_stas2)
var(dt_stas2)
```
...
Berdasarkan uji ADF tersebut, didapat *p-value* sebesar 0.01414 yang lebih kecil dari taraf nyata 5% sehingga tolak $H_0$ dan menandakan bahwa data partisi bagian kedua stasioner dalam rataan. Hal ini tidak sesuai dengan hasil eksplorasi menggunakan plot time series dan plot ACF.
...
Gambar di atas menunjukkan nilai *rounded value* ($\lambda$) optimum sebesar **0.36** dan pada selang kepercayaan 95% nilai memiliki batas bawah **0.00** dan batas atas **1.52**. Selang tersebut memuat nilai satu sehingga dapat dikatakan bahwa data partisi bagian kedua stasioner dalam ragam.
